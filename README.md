...
├── data/site-meta.json     마지막 Routine 실행 시각
├── scripts/
...
평일 (월-금) 09:00 KST
  Claude Desktop (Cowork 루틴)
    └─ GitHub API 로 anthropics/claude-code 신규 릴리즈 조회
        └─ data/site-meta.json 에 Routine 실행 시각 기록
            └─ 신규 릴리즈가 있으면 개발자 관점 요약 생성 (한국어)
...
`data/site-meta.json` 은 사이트 단위 메타데이터입니다. 상단의 "마지막 업데이트"는
릴리즈별 `summarizedAt` 이 아니라 이 파일의 `lastRoutineRunAt` 을 우선 사용합니다.
Routine 이 정상 실행될 때마다 `scripts/mark-routine-run.mjs` 가 값을 갱신합니다.
