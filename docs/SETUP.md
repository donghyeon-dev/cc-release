...
6. **Schedule** 설정:
   - Frequency: `Weekdays` (Mon-Fri) — 주간 옵션이 없으면 cron `0 9 * * 1-5`
   - Time: `09:00`
   - Timezone: `Asia/Seoul` (KST)
   - **주말 제외 필수**: Cowork 월간 루틴 실행 한도 25회 (Pro). 매일 돌리면 초과.
7. **Connectors** 활성화:
...
- Cowork Pro 기준 월 25회 제한. 평일 월-금 이면 월 20~22회로 여유 있음.
- 수동 "Run now" 도 카운트에 포함될 수 있음 → 테스트는 월 초에 1~2회로 제한.
- 한도 도달 시 다음 달 초까지 자동 실행은 중단되지만, 레포의 기존 데이터는 그대로 유지.
...
- [ ] 5. Claude Desktop Cowork 루틴 등록 (평일 09:00 KST, cron `0 9 * * 1-5`)
