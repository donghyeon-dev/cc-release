...
4. **분류 기준**
   ...
   - 해당 없으면 빈 배열 `[]`.
5. **`devImpact` 작성 기준**
   - `v2.0.73` 이상: `DevImpactItem[]` 배열. 각 원소는 `{ text, refs }`.
     - `text`: 영향 문장 (한국어). 한 문장 단위로 분리해 여러 원소로 구성.
     - `refs`: 근거 bullet 참조 `[{ bucket: "newFeatures"|"changes"|"fixes", index: N }]`.
       - 직접 대응 bullet 이 있으면 반드시 포함, 여러 개면 모두 나열.
       - 직접 대응이 없으면 `refs: []` (주석 번호 미표시).
     - 해당 없으면 **빈 배열 `[]`**.
   - `v2.0.72` 이하: 문자열 그대로 허용 (`""` 포함).
   - **다음 중 하나가 실제로 있을 때만** 서술:
     - breaking change / 마이그레이션 필요
     - 기본값 변경 (사용자가 인지해야 할 만한 것)
     - 환경변수·설정키 신설·삭제
     - 보안·권한 동작 변경
     - 특정 플랫폼 (Windows/Linux/macOS/Bedrock/VSCode 등) 한정 영향
   - 위 중 어느 것에도 해당 안 되면 **빈 배열 `[]`** (v2.0.72 이하는 `""`). 억지로 채우지 말 것.
   - **devImpact 금지 문구** (이런 식으로 쓸 거면 차라리 빈 배열):
     - "환경 변수나 설정 경로가 동작에 영향을 줄 수 있으므로 팀 공용 설정과 배포 환경을 확인해야 한다"
     - "권한 판정이나 샌드박스 정책이 달라질 수 있어 기존 allow/deny 규칙과 자동 승인 흐름을 재검토해야 한다"
   - 이런 **모든 릴리즈에 복붙 가능한 일반론**은 가치 없음. 빈 배열이 낫다.
   - **좋은 devImpact 예** (v2.0.73 이상):
     ```
     "devImpact": [
       {
       ...
       {
         "text": "응답 품질은 올라가지만 토큰 사용량이 기존 대비 증가할 수 있어 비용 모니터링 권장.",
         "refs": [{ "bucket": "changes", "index": 5 }]
       }
     ]
     ```
...
12. `devImpact` 가 v2.0.73+ 이면 배열, v2.0.72 이하면 문자열 또는 배열 허용.
    해당 내용이 없으면 v2.0.73+ 은 `[]`, v2.0.72 이하는 `""` (마지못해 채우지 말 것).
    배열 원소는 `{ text: string, refs: Array<{ bucket, index }> }` 구조 엄수.
